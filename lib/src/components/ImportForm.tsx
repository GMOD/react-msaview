...
export default observer(({ model }: { model: MsaViewModel }) => {
  ...
  const [lineageFile, setLinearFile] = useState<FileLocation>()
  const { error } = model
    ...
    <Container>
      ...
      <Grid container spacing={10} justify="center" alignItems="center">
        <Grid item>
          ...
          <Typography>Lineage colors file</Typography>
          <FileSelector location={lineageFile} setLocation={setLinearFile} />
        </Grid>
...
const Examples = ({ model }: { model: MsaViewModel }) => {
  return (
    <ul>
      <ListItem
        model={model}
