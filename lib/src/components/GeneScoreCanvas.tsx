...
const GeneScoreCanvas = observer(({ model }: { model: MsaViewModel }) => {
  ...
  return (
    ...
    >
      {blocksY.map(block => (
        <GeneScoreBlock key={block} model={model} offsetY={block} />
      ))}
    </div>
  )
})

export default GeneScoreCanvas
